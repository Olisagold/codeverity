"""GitHub sign-in: the login redirect, state validation, user/org creation on
first sign-in, and the one-time exchange code.

`fetch_profile` (the actual call to GitHub) is monkeypatched everywhere here;
nothing in this file talks to the real GitHub API. Postgres and Redis are the
...
def _fake_profile(monkeypatch, profile: GithubProfile) -> None:
    async def fake_fetch_profile(code: str) -> GithubProfile:
        return profile

    monkeypatch.setattr(auth_routes, "fetch_github_profile", fake_fetch_profile)


def _new_profile() -> GithubProfile:
...
def _get_state(client) -> str:
    return _query_param(client.get("/v1/auth/github/login").headers["location"], "state")


def _do_callback(client, state: str):
    return client.get("/v1/auth/github/callback", params={"code": "fake-code", "state": state})


def test_github_login_redirects_to_github(client) -> None:
...
def test_github_callback_rejects_unknown_state(client) -> None:
    response = _do_callback(client, state="not-a-real-state")
    assert response.status_code == 400


def test_first_time_github_signup_creates_org_and_issues_tokens(client, monkeypatch) -> None:
...
def test_returning_github_user_logs_in_without_a_new_org(client, monkeypatch) -> None:
    profile = _new_profile()
    email = profile.email
    _fake_profile(monkeypatch, profile)
